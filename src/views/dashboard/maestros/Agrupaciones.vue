    ...
    <v-row justify="center">
      ...
      >
        <base-material-card
          ...
          color="primary"
          title="Registro de Agrupaciones"
          class="px-5 py-3"
        >
          <v-spacer />
          <v-col
          ...
          <template v-slot:item.itemstatus="{ item }">
            ...
            <template v-slot:item.itemfamilia="{ item }">
              <v-switch
                v-model="item.familia"
                @change="comprobacion(item)"
              ></v-switch>
            </template>
          ...
          >
            ...
            <v-card>
              ...
              >
                ...
                <v-toolbar-title>
                  <span>Crear Nueva Agrupación</span>
                </v-toolbar-title>
                <v-spacer />
                <v-toolbar-items>
                  <v-btn
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        tabla_headers: [
          ...
          {
            text: 'Familia', value: 'itemfamilia',
          },
          {
    ...
    methods: {
      ...
      loadItems () {
        this.tabla_items = []
        axios.get(this.$session.get('url') + 'planificacion/agrupacion/',
      ...
      comprobacion (item) {
        axios.get(window.location.protocol + '//' + window.location.hostname + ':' + this.$session.get('puerto') + 'planificacion/agrupacion/?familia=true',
                  { headers: { Authorization: 'JWT ' + this.$session.get('token') } })
          .then((response) => {
            ...
            if (response.data[0]) {
              if (response.data[0].id == item.id ) {
                ...
                this.$swal({
                text: 'YA SE ESTA USANDO UNA FAMILIA',
      ...
      editItem(item){
        ...
        let method = 'patch'
        let url = this.$session.get('url') + 'planificacion/agrupacion/'
        const id = item.id
        item.user_update = this.$session.get('iduser')
        url = this.$session.get('url') + 'planificacion/agrupacion/' + id + '/'
        console.log(item)
      ...
      deleteItem (item) {
        this.$swal({
          ...
          if (result.isConfirmed) {
            axios.delete(window.location.protocol + '//' + window.location.hostname + ':' + this.$session.get('puerto') + 'planificacion/agrupacion/' + item.id + '',
                         { headers: { Authorization: 'JWT ' + this.$session.get('token') } })
              .then((response) => {
